...
def main():
    ...
    if args.s1 == 1:
        ...
        run_name = "1"
        device = args.d1
        trainset = load_data(library, data_folder, "lorenz/scale-1.0", t, dt, model)
        pipeline(library=library, trainset=trainset, epochs=999, lr=5e-3, batch_size=250,
            ...
            beta_init=0.01, beta_max=10.0, beta_max_epoch=100, beta_spike=100.0, beta_spike_epoch=400,
            noise_dim=6, thresh_interval=100, hard_thresh=0.05, batch_norm=False, eval_interval=50,
    ...
    if args.s2 == 1:
        ...
        run_name = "2"
        device = args.d2
        trainset = load_data(library, data_folder, "lorenz/scale-5.0", t, dt, model)
        pipeline(library=library, trainset=trainset, epochs=999, lr=5e-3, batch_size=250,
            lmda_init=1e-2, lmda_max=1e-2, lmda_max_epoch=1, lmda_spike=1e1, lmda_spike_epoch=400,
            beta_init=0.01, beta_max=10.0, beta_max_epoch=100, beta_spike=400.0, beta_spike_epoch=400,
            noise_dim=6, thresh_interval=100, hard_thresh=0.05, batch_norm=False, eval_interval=50,
            adam_reg=adam_reg, gamma_factor=gamma_factor, runs=runs, run_name=run_name, device=device,
            hidden_dim=hidden_dim, stat_size=stat_size, num_hidden=num_hidden)
    ...
    if args.s3 == 1:
        ...
        run_name = "3"
        device = args.d3
        trainset = load_data(library, data_folder, "lorenz/scale-10.0", t, dt, model)
        pipeline(library=library, trainset=trainset, epochs=999, lr=5e-3, batch_size=250,
            lmda_init=1e-2, lmda_max=1e-2, lmda_max_epoch=1, lmda_spike=1e1, lmda_spike_epoch=400,
            beta_init=0.01, beta_max=10.0, beta_max_epoch=100, beta_spike=400.0, beta_spike_epoch=400,
            noise_dim=6, thresh_interval=100, hard_thresh=0.05, batch_norm=False, eval_interval=50,
