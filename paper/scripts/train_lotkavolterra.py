...
"""
Train HyperSINDy on lotka-volterra 
"""
...
def main():
    ...
    if args.s1 == 1:
        ...
        run_name = "1"
        device = args.d1
        trainset = load_data(library, data_folder, "lotkavolterra/scale-5.0", t, dt, model)
        pipeline(library=library, trainset=trainset, epochs=499, lr=5e-3, batch_size=250,
            lmda_init=1e-2, lmda_max=1e-2, lmda_max_epoch=1, lmda_spike=1e2, lmda_spike_epoch=200,
            beta_init=0.01, beta_max=10.0, beta_max_epoch=100, beta_spike=300, beta_spike_epoch=200,
            noise_dim=4, thresh_interval=100, hard_thresh=0.05, batch_norm=False, eval_interval=50,
